...
contract OptionsOccupiedCal is OptionsBase {
    ...
    function setCollateralPhase(uint256 totalCallOccupied,uint256 totalPutOccupied,uint256 beginOption,
        ...
        require(latestCallOccpied>=-1e50 && latestCallOccpied<=1e50,"options fall calculate error");
        require(latestPutOccpied>=-1e50 && latestPutOccpied<=1e50,"options fall calculate error");
...
//        OptionsInfoEx storage infoEx =  optionExtraMap[optionID-1];
        uint256 newOccupied = calOptionsCollateral(info,info.underlyingPrice);
        if (info.optType == 0){
